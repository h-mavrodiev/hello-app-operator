...
//+kubebuilder:rbac:groups=hp.mavrodiev,resources=helloapps,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=hp.mavrodiev,resources=helloapps/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=hp.mavrodiev,resources=helloapps/finalizers,verbs=update
...
func (r *HelloAppReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	logger := log.FromContext(ctx).WithValues("HelloApp", req.NamespacedName)
	...
	if result != nil {
		logger.Error(err, "Service Not Ready")
		return *result, err
	}
